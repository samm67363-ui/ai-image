...
that ResNet18 expects, using the exact transforms that ship with
the pretrained weights (resize -> center crop -> tensor -> normalize).
"""
...
def preprocess_image(file_bytes: bytes, weights) -> torch.Tensor:
    """
    Converts raw image bytes into a batched tensor ready for the model.

    `weights` is the ResNet18_Weights enum instance (passed in from
    model_loader.get_weights()) — it exposes `.transforms()` which
    returns the exact resize/crop/normalize pipeline used when the
